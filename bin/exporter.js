  ...
  return {
    copyWebsiteContent: function({ html, slugFolderName }) {
      return new Promise(function(resolve, reject) {
        ...
        let cacheFolderName = api.getCurrentDate() + slugFolderName + '-' + (Math.random().toString(36)+'00000000000000000').slice(2, 3 + 2);

        let cachePath = path.join(global.appRoot, settings.cacheDirname, cacheFolderName);
        ...
        fs.mkdirp(cachePath, function() {
          ...
          tasks.push(
            new Promise((resolve, reject) => {
              let productionFolder = path.join(global.appRoot, 'client', 'production');
