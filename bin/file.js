...
module.exports = (function() {
    ...
    return new Promise(function(resolve, reject) {
      ...
      fs.access(presentationMd, fs.F_OK, function(err) {
        if(err) {
          ...
          let presentationContent = validator.unescape(fs.readFileSync(presentationMd, settings.textEncoding));
  ...
  function getMetaFileOfFolder(slugFolderName) {
    ...
    let metaPath = path.join(folderPath, settings.folderMetafilename + settings.metaFileext);
    return metaPath;
  }
      ...
      fs.access(potentialMetaFile, fs.F_OK, (err) => {
        ...
        Promise.all(tasks).then(mediaData => {
          ...
          if(mediaData.type === 'text' || mediaData.type === 'marker') {
            // get text content
            let mediaPath = path.join(api.getFolderPath(slugFolderName), slugMediaName);
            mediaData.content = validator.unescape(fs.readFileSync(mediaPath, settings.textEncoding));
...
//         dev.logverbose(`Found filenames: ${filenames}`);
        ...
        var folders = filenames.filter(function(thisSlugFolderName) {
            // is a folder
          return new RegExp(settings.regexpMatchFolderNames, 'i').test(thisSlugFolderName) &&
  ...
  function removeFolder(slugFolderName) {
    return new Promise(function(resolve, reject) {
      ...
      let folderPath = api.getFolderPath(slugFolderName);
      let movedFolderPath = path.join(api.getFolderPath(), settings.deletedFolderName, slugFolderName);
    ...
    return new Promise(function(resolve, reject) {
      ...
      fs.readdir(slugFolderPath, function (err, filenames) {
        ...
        let medias = filenames.filter(function(thisSlugMediaName){
            // not a folder
          return !new RegExp( settings.regexpMatchFolderNames, 'i').test(thisSlugMediaName) &&
            // not meta.txt
            thisSlugMediaName !== settings.folderMetafilename + settings.metaFileext &&
            // not a text file
            new RegExp( settings.regexpGetFileExtension, 'i').exec(thisSlugMediaName)[0] !== '.txt' &&
            // not deleted
      ...
      fs.access(potentialMetaFile, fs.F_OK, function(err) {
        ...
        if(err) {
          ...
          if(additionalMeta !== undefined && additionalMeta.hasOwnProperty('type')) {
            ...
            let mediaFileExtension = new RegExp(settings.regexpGetFileExtension, 'i').exec(slugMediaName)[0];
    ...
    return new Promise(function(resolve, reject) {
      ...
      **************************************************************************/
      ...
      readOrCreateMediaMeta(slugFolderName,slugMediaName).then((meta) => {
        dev.logverbose(`Got meta, now updating for ${slugMediaName} with ${JSON.stringify(meta, null, 4)}`);
        ...
        if((meta.type === 'text' || meta.type === 'marker') && mdata.hasOwnProperty('content')) {
          ...
          let updateTextMedia = new Promise((resolve, reject) => {
            ...
            api.storeData(mediaPath, content, 'update').then((content) => {
              dev.logverbose(`Updated media file at path: ${mediaPath} with content: ${content}`);
              resolve();
            }).catch(err => {
              reject(err);
            });
  ...
  function removeMedia(slugFolderName, slugMediaName) {
    return new Promise(function(resolve, reject) {
      ...
      let mediaPath = path.join(api.getFolderPath(slugFolderName), slugMediaName);
      let movedMediaPath = path.join(api.getFolderPath(slugFolderName), settings.deletedFolderName, slugMediaName);
